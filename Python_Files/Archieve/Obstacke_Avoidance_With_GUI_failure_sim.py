    ...
    def run(self):
        ...
        while t < max_t:
            ...
            if self.localized_pbd:
                # CRITICAL FIX: The physics broad-phase MUST use physical margins,
                # not the A* perception frame. Otherwise, highly compressed vehicles
                # (like PBD_A, B, C) are ignored by physics if they are > 8m apart!
                danger_pairs = []
                for i in range(n_veh):
                    vi = active_veh[i]
                    for j in range(i+1, n_veh):
                        ...
                        # Generous Physics Window (Combined Margin + 15m)
                        if dist <= (vi.margin + vj.margin + 15.0):
...
# ==========================================
    def verify_manifest(self):
        ...
        print(f"PBD Mode: {'LOCALIZED (Physics Frame)' if self.localized_pbd else 'GLOBAL (Entire Map)'}")
        print("="*80)
...
# ==========================================
    def plot(self):
        ...
        pbd_title_text = "LOCALIZED (Physics Frame)" if self.localized_pbd else "GLOBAL (Entire Map)"
